 ...
 * one place. A column the map paints red is a column the findings list has a
 * critical entry for, because both use the same evaluation.
 */
export function MapView() {
  const plan = useActivePlan()
  const map = useMemo(() => (plan ? buildMap(createContext(plan)) : null), [plan])
  const index = useMemo(() => (plan ? indexPlan(plan) : null), [plan])

  if (!plan || !map || !index) return null
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <ViewHeader
        eyebrow="Diagnosis"
        title="Map"
        question="Keys down the side, places across the top. The row at the bottom is the one that matters: how much of each wallet is inside one container."
      />
